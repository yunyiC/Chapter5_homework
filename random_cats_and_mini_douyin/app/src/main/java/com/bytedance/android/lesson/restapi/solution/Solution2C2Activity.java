...
public class Solution2C2Activity extends AppCompatActivity {
    ...
    private void postVideo() {
        ...
        Log.d(TAG, "partVideo: " + partVideo.toString() + "partImage : " + partImage.toString());
        Call<PostVideoResponse> callPost = NetworkUtils.getResponseWithRetrofitAsyncPost("0", "A", partImage, partVideo);
        mCallListPost.add(callPost);
        callPost.enqueue(new Callback<PostVideoResponse>() {
            @Override
            public void onResponse(Call<PostVideoResponse> call, Response<PostVideoResponse> response) {
                if (response.isSuccessful()) {
                    Toast.makeText(Solution2C2Activity.this, "sent succeddfully", Toast.LENGTH_SHORT).show();
                    Log.d(TAG, "onResponse: Successfully");
                } else {
                    Toast.makeText(Solution2C2Activity.this, "sent Failed", Toast.LENGTH_SHORT).show();
                    Log.d(TAG, "onResponse: Failed");
                }
                resetPostBtn();
                mCallListFeed.remove(call);
            }
    ...
    public void fetchFeed(View view) {
        ...
        Call<FeedResponse> callFeed = NetworkUtils.getResponseWithRetrofitAsyncFeed();
        mCallListFeed.add(callFeed);
        callFeed.enqueue(new Callback<FeedResponse>() {
            @Override
